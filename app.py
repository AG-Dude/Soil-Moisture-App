...
def init_ee_from_env():
    ...
    if not key or not key.strip():
        st.error(
            "EE_PRIVATE_KEY is missing or empty.\n\n"
    ...
    # If the JSON was pasted wrapped in one pair of quotes, strip them once
    ...
    try:
        ...
        st.error(
            f"EE_PRIVATE_KEY is not valid JSON: {e}\n\n"
    ...
    if missing:
        st.error(
            f"Service-account JSON is missing fields: {missing}. "
...
try:
    s1 = (
        ...
        # Property is an array → use listContains for 'VV'
